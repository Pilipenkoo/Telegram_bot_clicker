...
u_admin = [737880162,5580265986,1109894249]
click = 0
...
#Хендлер на текстовое сообщение
...
async def echo(message: types.Message):
    if message.text == click_text:
        ...
        # Увеличиваем баланс пользователя на 0.25 монет
        user_id = message.from_user.id
        balance= main.about_balance(user_id)
        ...
        if click % 3 == 0:
                ...
                if file_id[0] is not None: 
                    # Отправляем сообщение с фото и подписью пользователю
            ...
            # отправляем сообщение с новым балансом
           await bot.send_message(message.chat.id,f'Следующий клик будет доступен через 1 секунду 🔥Ваш баланс на данный момент:  {str(balance[0])} ₽',reply_markup=kb_rep)        
...
async def callback(callback_query: types.CallbackQuery):
    if callback_query.data == 'click':
        ...
        # btn4 = telebot.types.InlineKeyboardButton(text='Админ_панель')
        kb_rep.add(btn1,btn_back)  
...
async def process_photo(message: types.Message, state: FSMContext):
    # Сохраняем file_id фотографии в состоянии
...
async def process_caption(message: types.Message, state: FSMContext):
    # Получаем file_id и подпись из состояния
